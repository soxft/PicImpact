...
export default function MasonryItem() {
  ...
  const { data: download = false, mutate: setDownload } = useSWR(
    ["masonry/download", MasonryViewData.url],
    null
  );
  ...
  const loadingHandle = React.useCallback(
    async (handle: string) => {
      const idx = MasonryViewDataList.findIndex(
        (item: ImageType) => MasonryViewData.id === item.id
      );
      if (handle === "next" && idx === MasonryViewDataList.length - 1) {
        setMasonryViewData(MasonryViewDataList[0] || MasonryViewData);
      } else {
        const [prev, next] = [
          MasonryViewDataList.at(idx - 1),
          MasonryViewDataList.at(idx + 1),
        ];
        if (handle === "prev") {
  ...
  React.useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      if (
        tabsListRef.current &&
        tabsListRef.current.contains(e.target as Node)
      ) {
        return;
      }
          ...
          <div className="flex w-full flex-col">
            ...
            >
                    ...
                    <Card className="py-4 show-up-motion">
                      <div className="pb-0 pt-2 px-4 flex-col items-start">
                        <div className="flex items-center space-x-1">
                          <CircleAlert size={20} />
                          <p className="text-tiny uppercase font-bold select-none">
                            转载提示
                          </p>
                        </div>
                        <h4 className="font-bold text-large">
                          本作品由作者版权所有，未经授权禁止转载、下载及使用。
                        </h4>
                      </div>
                    </Card>
                  )}
                      ...
                      <Card className="py-2 show-up-motion">
                        <div className="pb-0 pt-2 px-2 flex-col items-start space-y-2">
                          <div className="flex items-center justify-center space-x-1 w-full">
                            <Camera size={20} />
                            <p className="text-tiny uppercase font-bold select-none items-center justify-center">
                              {`${
                                MasonryViewData?.exif?.make
                              } ${MasonryViewData?.exif?.model.replace(
                                MasonryViewData?.exif?.make,
                                ""
                              )}`}
                            </p>
              ...
              <TabsContent value="copyright">
                <div className="flex flex-col space-y-2">
                  {Array.isArray(MasonryViewData.copyrights) &&
                  MasonryViewData.copyrights.length > 0 ? (
                    <div className="flex flex-col space-y-2 mt-2 w-full">
                      {MasonryViewData.copyrights.map(
                        (copyright: CopyrightType) => {
                          if (copyright.type === "social") {
                            return (
                              <Card
                                key={copyright.id}
                                className="flex h-32 flex-col show-up-motion justify-center"
                              >
                                <div className="flex justify-between w-full p-2 space-x-2">
                                  <div className="flex justify-center items-center space-x-2">
                                    <Avatar>
                                      <AvatarImage
                                        src={copyright.avatar_url}
                                        alt="avatar"
                                      />
                                    ...
                                    <div className="flex flex-col gap-1 items-start justify-center">
                                      ...
                                      <h5 className="text-small tracking-tight text-default-400">
                                        {copyright.social_name}
                                      </h5>
                                    </div>
                                  </div>
                                  <Link href={copyright.url} target="_blank">
                                    <Button>Follow</Button>
                                  </Link>
                                </div>
                                <div className="flex flex-1 px-3 py-0 text-small text-default-400 overflow-y-auto scrollbar-hide">
                                  <p>{copyright.detail}</p>
                                </div>
                              </Card>
                            );
                          }
